...
const LeidingAdminEditModal: React.FC<Props> = ({ leiding, onClose, onEdit }) => {
    ...
    const editLeiding = async () => {
        ...
        try {
            ...
            if (isAdmin && !isEditingOwnProfile) {
                updatedLeiding = await LeidingService.changeRole(leiding.id, editedLeiding.rol);
                await LeidingService.changeGroup(leiding.id, editedLeiding.groep);
            } else {
...
export default LeidingAdminEditModal;